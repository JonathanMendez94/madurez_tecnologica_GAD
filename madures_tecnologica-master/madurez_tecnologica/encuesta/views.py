...
def index2(request):
    if request.user.is_authenticated:
        ...
        lastnme = current_user.last_name
        messages.success(request, 'Bienvenido '+username+' '+lastnme)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            fichas2 = Fichas.objects.filter(gad_servi_usuario__gad_ser__gad=valor2['id_gad']). \
                ...
                annotate(Top_Down_Total=ExpressionWrapper(F('Top_Down') / F('valB'), output_field=FloatField())).                \
...
def grafica(request, cuestionario, GadServ):
    ficha = (Fichas.objects.filter(gad_servi_usuario__gad_ser__idgadservicio=GadServ,
             ...
             annotate(Total=Sum('pregunta_respuesta__valor'),
                      valB=Count('gad_servi_usuario')). \
             annotate(val = ExpressionWrapper((F('pregunta_respuesta__pregunta__encuesta__valor')-(F('Total')/F('valB'))-(F('Total')/F('valB'))), output_field=FloatField())))
